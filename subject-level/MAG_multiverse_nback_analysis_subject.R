...
#   multiverse_nback_subject_results.csv
#   multiverse_nback_subject_conditions_results.csv
...
if (nrow(M_interaction) > 0) {
  write.csv(M_interaction, file.path(csv_dir, "multiverse_nback_subject_interaction_results.csv"), row.names = FALSE)
  message(sprintf("Saved: multiverse_nback_subject_interaction_results.csv (%d universes)", nrow(M_interaction)))
} else {
...
if (nrow(M_ca) > 0L) {
  M_ca <- add_sig(M_ca)
  write.csv(M_ca, file.path(csv_dir, "multiverse_nback_subject_condition_results.csv"), row.names = FALSE)
  message(sprintf("Saved: multiverse_nback_subject_condition_results.csv (%d universes)", nrow(M_ca)))
} else {
...
if (nrow(M_cg) > 0L) {
  M_cg <- add_sig(M_cg)
  write.csv(M_cg, file.path(csv_dir, "multiverse_nback_subject_condition_gaze_results.csv"), row.names = FALSE)
  message(sprintf("Saved: multiverse_nback_subject_condition_gaze_results.csv (%d universes)", nrow(M_cg)))
} else {
