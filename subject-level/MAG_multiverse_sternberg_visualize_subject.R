# MAG Multiverse — Sternberg: Subject-Level Visualization
...
#
# Requires: MAG_multiverse_sternberg_analysis_subject.R to have been run first.
...
cond_path <- file.path(csv_dir, "MAG_multiverse_sternberg_subject_conditions_results.csv")
if (!file.exists(cond_path)) stop("Conditions CSV not found: ", cond_path,
                                   "\nRun MAG_multiverse_sternberg_analysis_subject.R first.")
...
ggsave(file.path(storage_plot, "MAG_multiverse_sternberg_subject_grouped.png"),
       plot = p_grp_combined, width = 14, height = 12, dpi = 600, bg = "white")
message("Saved: MAG_multiverse_sternberg_subject_grouped.png")

# ========== FIGURE 2: CONDITION -> ALPHA (EEG-only) ==========
ca_path <- file.path(csv_dir, "MAG_multiverse_sternberg_subject_condition_results.csv")
if (file.exists(ca_path)) {
  ...
  ggsave(file.path(storage_plot, "MAG_multiverse_sternberg_subject_condition_alpha.png"),
         plot = p_ca_combined, width = 14, height = 10, dpi = 600, bg = "white")
  message("Saved: MAG_multiverse_sternberg_subject_condition_alpha.png")
} else {
  message("Skipping Figure 2: subject_condition_results.csv not found.")
}

# ========== FIGURE 3: INTERACTION (gaze x condition -> alpha) ==========
int_path <- file.path(csv_dir, "MAG_multiverse_sternberg_subject_interaction_results.csv")
if (file.exists(int_path)) {
  ...
  p_int_combined <- p_int_curve / legend_int / p_int_panel +
    plot_layout(heights = c(0.8, 0.1, 1.5))
  ggsave(file.path(storage_plot, "MAG_multiverse_sternberg_subject_interaction.png"),
         plot = p_int_combined, width = 14, height = 12, dpi = 600, bg = "white")
  message("Saved: MAG_multiverse_sternberg_subject_interaction.png")
} else {
  message("Skipping Figure 3: subject_interaction_results.csv not found.")
}

# ========== FIGURE 4: CONDITION -> GAZE (gaze-only) ==========
cg_path <- file.path(csv_dir, "MAG_multiverse_sternberg_subject_condition_gaze_results.csv")
if (file.exists(cg_path)) {
  ...
  ggsave(file.path(storage_plot, "MAG_multiverse_sternberg_subject_condition_gaze.png"),
         plot = p_cg_combined, width = 14, height = 8, dpi = 600, bg = "white")
  message("Saved: MAG_multiverse_sternberg_subject_condition_gaze.png")
} else {
...
ap_gaze_path <- file.path(csv_dir, "MAG_multiverse_sternberg_subject_aperiodic_gaze_results.csv")
...
if (has_ap_gaze) {
  ...
  for (ap_measure in c("Exponent", "Offset")) {
    ...
    fname <- paste0("MAG_multiverse_sternberg_subject_aperiodic_",
                    suffix, "_spec.png")
