# AOC Multiverse — Sternberg: Subject-Level Visualization
...
cond_path <- file.path(csv_dir, "multiverse_sternberg_subject_conditions_results.csv")
if (!file.exists(cond_path)) stop("Conditions CSV not found: ", cond_path,
                                   "\nRun AOC_multiverse_sternberg_analysis_subject.R first.")
...
ggsave(file.path(storage_plot, "AOC_multiverse_sternberg_subject_grouped.png"),
...
if (file.exists(ca_path)) {
  ...
  ggsave(file.path(storage_plot, "AOC_multiverse_sternberg_subject_condition_alpha.png"),
...
if (file.exists(int_path)) {
  ...
  p_int_combined <- p_int_curve / legend_int / p_int_panel +
    plot_layout(heights = c(0.8, 0.1, 1.5))
  ggsave(file.path(storage_plot, "AOC_multiverse_sternberg_subject_interaction.png"),
...
if (file.exists(cg_path)) {
  ...
  ggsave(file.path(storage_plot, "AOC_multiverse_sternberg_subject_condition_gaze.png"),
...
if (has_ap_gaze) {
  ...
  for (ap_measure in c("Exponent", "Offset")) {
    ...
    fname <- paste0("AOC_multiverse_sternberg_subject_aperiodic_",
                    suffix, "_spec.png")
